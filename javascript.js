...
function GameController (
    ...
    const checkForWinner = (board, token) => {
    ...
    const restartBoard = () => {
        for (let i = 0; i < 3; i++) {
            for (let j = 0; j < 3; j++) {
                board.getBoard()[i][j] = Cell(); // Reset each cell to an empty Cell
            }
        }
    ...
    printNewRound();
    return {playRound, getActivePlayer, getBoard: board.getBoard, restartBoard};
}
...
function ScreenController() {
    ...
    function clickHandlerBoard(e) {
        ...
        if (winnerFlag || drawCount === 9){return;}
